...
export function ExecutionStepper() {
    ...
    const currentPath = pathname.split('/execution')[1] || '';
    const currentIndex = executionSteps.findIndex(step => currentPath.startsWith(step.href));
        ...
        <div className="border-b bg-card">
            <nav className="container mx-auto -mb-px flex space-x-8 px-4" aria-label="Tabs">
                {executionSteps.map((step, index) => {
                    ...
                    <Link
                        key={step.name}
                        href={`/projects/${projectId}/execution${step.href}`}
                        className={cn(
